...
if (CModule::IncludeModule('iblock')) {
	...
	$i_user=0;
	while($obUser = $resUser->GetNext())
	{
		$ArrRegions[$obUser['ID']] = $obUser;
...
<div id="content">
	...
	<div class="content_right">				
		...
		if(CModule::IncludeModule("iblock"))
		{
			...
			$ar_result=CIBlockElement::GetList(Array("NAME"=>"ASC"), Array("IBLOCK_ID"=>$iblock_id, "PROPERTY_AFFILIATE_VALUE"=>"Нет", "ID"=>array(2176,840)), $arSelect);
			$alfab_symbol="";
			while($res=$ar_result->GetNext())
			{
					...
					<div class="descriptions">
						...
						while($res2=$ar_result2->GetNext())
						{
							...
							$end='yes';
								...
								$db_groups = CIBlockElement::GetElementGroups($res2['PROPERTY_CML2_LINK_VALUE'], true);
								$db_groups->SetUrlTemplates();
								while($ar_group = $db_groups->GetNext()) {
									if (empty($RegionsList[$res['ID'][$ar_group['ID']]])) {
										$RegionsList[$res['ID']][$ar_group['NAME']]['NAME'] = $ar_group['NAME'];
										$RegionsList[$res['ID']][$ar_group['NAME']]['SECTION_PAGE_URL'] = $ar_group['SECTION_PAGE_URL'];
										$RegionsList[$res['ID']][$ar_group['NAME']]['GET_PARAM'] = $res2['NAME'];
									}
								}
				
						}?>

					<? if (!empty($RegionsList[$res['ID']])) {?> 
					<?php asort($RegionsList[$res['ID']])?>
						<div id="affiliates">
							<div class="affiliates_title"><span>Филиалы</span></div>
							<? foreach ($RegionsList[$res['ID']] as $region) {?>
								<div class="affiliates_wrap">
									<a href="<?=$region["SECTION_PAGE_URL"];?>?PARENT=<?=$res['ID'];?>" class="LinkDest"><?=$region["NAME"];?></a> 	
								</div>
							<?}?>
