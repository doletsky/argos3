...
$db_vals = CSaleOrder::GetByID($ORDER_ID);
...
//Комментарии к счёту
if ((GetParam("COMMENT1", $PSinfo) && $days == 1) || GetParam("COMMENT2", $PSinfo))
{
...
if ($arBasket = $dbBasket->Fetch())
{
	...
	$arColsCaption = array(
		...
		CSalePdf::prepareToPdf('Цена (с НДС) руб.'), //$currency
		CSalePdf::prepareToPdf('Ставка НДС'),
		CSalePdf::prepareToPdf('Сумма (с НДС) руб.') //$currency
	);
	$arCells = array();
	$arProps = array();
