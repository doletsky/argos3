	...
	<table class="data-table">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<tr<?if($propertyID==295 || $propertyID==296)echo' style="display:none;"'?>>
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								<?if($propertyID!=295 && $propertyID!=296) {?>
									<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" />
								<?
								}
								else {?>
									<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?if(isset($_GET['page'])) echo $_GET['page']; else echo 'all';?>" /><?//Устанавливаем идентификатор страницы?>
								<?}
			...
			<tr>
				<td>&nbsp;</td>
				<td>
					<label class="pseudo_check check marg_check reviews">
						<input type="checkbox" name="reviews_subscriber" id="reviews_subscriber" style="" checked="checked" />
						<?=GetMessage("NL_AGREE")?>
					</label>
				</td>
			</tr>
			<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
				<tr>
					<td><?=GetMessage("IBLOCK_FORM_CAPTCHA_TITLE")?><span class="starrequired">*</span></td>
					<td>
						<input type="hidden" name="captcha_sid" value="<?=$arResult["CAPTCHA_CODE"]?>" />
						<img class="captcha" src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["CAPTCHA_CODE"]?>" width="180" height="40" alt="CAPTCHA" style="" />
						<input class="min" type="text" name="captcha_word" maxlength="50" value="">
					</td>
				</tr>
				<?/*<tr>
					<td><?=GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT")?><span class="starrequired">*</span>:</td>
					<td><input class="min" type="text" name="captcha_word" maxlength="50" value=""></td>
				</tr>*/?>
			<?endif?>
		</tbody>
		<?endif?>
