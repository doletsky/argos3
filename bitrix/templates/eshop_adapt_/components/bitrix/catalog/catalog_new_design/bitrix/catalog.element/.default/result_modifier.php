...
if(!empty($arResult['PROPERTIES']['PDF_OFFERS_TECHNICAL_CHARACTERISTICS']['VALUE']) && isset($arResult['PROPERTIES']['PDF_OFFERS_TECHNICAL_CHARACTERISTICS']['VALUE']) ){
	$rsFile = CFile::GetByID($arResult['PROPERTIES']['PDF_OFFERS_TECHNICAL_CHARACTERISTICS']['VALUE']);
	$arFile = $rsFile->Fetch();
	$arFile['URL'] = '/upload/'.$arFile['SUBDIR'].'/'.$arFile['FILE_NAME'];
	///upload/iblock/d8b/zakaz_286.pdf
	$arResult['PROPERTIES']['PDF_OFFERS_TECHNICAL_CHARACTERISTICS']['VALUE'] = $arFile;
}


$arSECTION_CODELamps = array(
		'svetodiodnye_svetilniki_zhkkh',
		'seriya_domovykh_svetodiodnykh_svetilnikov_s_datchikom_intellekt_zhkkh_',
		'seriya_domovykh_svetilnikov_intellekt_zhkkh',
		'seriya_domovykh_led_svetilnikov_sfera_zhkkh',
		'standartnye_domovye_svetilniki'
);
$arSECTION_CODEDrivers = array('draivery','35_vt','40_vt','50_vt','60_vt','80_vt','ips_tu','ips_tp');
$arSECTION_CODESensors = array('datchiki','svetozvukovoy_datchik_szvo');
$arSECTION_CODEModules = array('moduli','seriya_seoul','seriya_epistar','promyshlenno_ulichnye_moduli');

$arPropIdLampsOffer = array(399,351,74,350,401,402,403);
$arPropIdDriversOffer = array(78,325,79,74,328,405,412,406,407,409,408);
$arPropIdDriversNew = array(
			0 => "DRIVER_CLASS",
			1 => "DEVIANT_PACKING",
			2 => "MAX_POWER",
			3 => "STABILIZING",
...
$arPropIdSensorsOffer = array(413,414,415,416);
$arPropIdModules = array(348,265,379,58,53,52,377,51,49,56,57,55);

if(in_array($arParams['SECTION_CODE'],$arSECTION_CODELamps)){
	foreach ($arResult['PROPERTIES'] as $propID => $propInfo):
		if(in_array($propInfo['ID'],$arPropIdLampsOffer)){
			$arResult['PROPS'][$propID] = $propInfo;
		}
	endforeach;
}
// - Новые драйверы
if(empty($arParams['SECTION_CODE']) ){
	foreach ($arResult['PROPERTIES'] as $propID => $propInfo):
		if(in_array($propID,$arPropIdDriversNew)){
			$arResult['PROPS'][$propID] = $propInfo;
		}
	endforeach;
}

if(in_array($arParams['SECTION_CODE'],$arSECTION_CODEDrivers) ){
	$res = CIBlockElement::GetByID($arResult['PROPERTIES']['MODEL']['VALUE']);
	if($ar_res = $res->GetNext()) $arResult['NAME'] = $ar_res['NAME'];
	
	foreach ($arResult['PROPERTIES'] as $propID => $propInfo):
		if(in_array($propInfo['ID'],$arPropIdDriversOffer)){
			$arResult['PROPS'][$propID] = $propInfo;
		}
	endforeach;
}

if(in_array($arParams['SECTION_CODE'],$arSECTION_CODESensors) ){
	foreach ($arResult['PROPERTIES'] as $propID => $propInfo):
		if(in_array($propInfo['ID'],$arPropIdSensorsOffer)){
			$arResult['PROPS'][$propID] = $propInfo;
		}
	endforeach;
}

if(in_array($arParams['SECTION_CODE'],$arSECTION_CODEModules) ){
	foreach ($arResult['PROPERTIES'] as $propID => $propInfo):
	if(in_array($propInfo['ID'],$arPropIdModules)){
		$arResult['PROPS'][$propID] = $propInfo;
	}
	endforeach;
}
