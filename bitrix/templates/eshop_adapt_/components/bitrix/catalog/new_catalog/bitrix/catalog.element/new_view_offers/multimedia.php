<div id="content" class="whole multi">
	...
	<div class="tabs_grey_style">
		...
		foreach($arResult['OFFERS'] as $offers)
		{
			...
			{
				$photos=$offers['PROPERTIES']['MULTIMEDIA_PHOTOS']['VALUE'];
				if($photos){
					$varphotos = true;
				}
				$promo=$offers['PROPERTIES']['MULTIMEDIA_PROMOTIONAL_MATERIALS']['VALUE'];
				if($promo){
					$varpromo = true;
				}
				$calc=$offers['PROPERTIES']['MULTIMEDIA_CALCULATOR']['VALUE'];
				if($calc){
					$varcalc = true;
				}
				$file=$offers['PROPERTIES']['FILE_3D']['VALUE'];
				if($file){
					$varfile = true;
				}				
				$arvideo=$offers['PROPERTIES']['MULTIMEDIA_VIDEO']['VALUE'];
				if($arvideo) {
					$videoprop = false;
		...
		foreach($arResult['OFFERS'] as $offers)
		{
			if($offers['ID']==$_GET['offers_id'])
			{
				$files_calculator=$offers['PROPERTIES']['MULTIMEDIA_PROMOTIONAL_MATERIALS']['VALUE'];
				if($files_calculator) {
					$inactive_class="";
				}
			}
		...
		foreach($arResult['OFFERS'] as $offers)
		{
			if($offers['ID']==$_GET['offers_id'])
			{
				$links_video=$offers['PROPERTIES']['MULTIMEDIA_VIDEO']['VALUE'];	
				if($links_video) {
		...
		foreach($arResult['OFFERS'] as $offers)
		{
			...
			if($offers['ID']==$_GET['offers_id'])
			{			
				$files_3d=$offers['PROPERTIES']['FILE_3D']['VALUE'];	
				if($files_3d) {
					$inactive_class="";
				}
			}
		...
		?>		
		<a <?if($inactive_class!="inactive") {?> href="?view=new&offers_id=<?=$_GET['offers_id']?>&tab=4&multimedia=5"<?}?> class="tab_grey_style tab_5<?if(isset($_GET['tab']) && $_GET['tab']=='4' && isset($_GET['multimedia']) && $_GET['multimedia']=='5')echo' current'?>"><span class="s_15 <?=$inactive_class?>"><?=GetMessage("MULTIMEDIA_BANNERS");?></span></a>
	</div>
	...
	else if(isset($_GET['tab']) && $_GET['tab']=='4' && isset($_GET['multimedia']) && $_GET['multimedia']=='4' || isset($_GET['tab']) && $_GET['tab']=='4' && !isset($_GET['multimedia']) && $resArr["calc"]==true)//Варианты применения
	{
	...
	}else if(isset($_GET['tab']) && $_GET['tab']=='4' && isset($_GET['multimedia']) && $_GET['multimedia']=='6' || isset($_GET['tab']) && $_GET['tab']=='4' && !isset($_GET['multimedia']) && $resArr["file"]==true)//3D
	{
