<div class="multimedia_body">
	...
	foreach($arResult['OFFERS'] as $offers)
	{
		...
		/*?><pre><?print_r($offers['PROPERTIES']);?></pre><?*/
			if($offers['ID']==$_GET['offers_id'])
			{
				/*$links_video_yt=$offers['PROPERTIES']['MULTIMEDIA_VIDEO_YOUTUBE']['VALUE'];	
				if($links_video_yt)
				{
					foreach ($links_video_yt as $link_video_yt_id)
					{
						?>
						<div style="margin:0 auto; margin-bottom:30px; width:640px;">
							<iframe width="640" height="360" src="//www.youtube.com/embed/<?=$link_video_yt_id?>" frameborder="0" allowfullscreen></iframe>
						</div>
						<?
					}
				}*/
					...
					foreach ($links_video as $video)
					{
						if($video) {
							...
							<div style="margin:0 auto; width:640px;">
								...
								<?}else{?>
									<a class="video_under" href="<?=$video['path']?>"><?=GetMessage("YT")?></a>
								<?}?>
							</div>
							<?
						}
