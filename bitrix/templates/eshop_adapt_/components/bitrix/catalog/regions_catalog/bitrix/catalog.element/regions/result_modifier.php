...
if (!empty($_REQUEST['NAME_DEST'])) {
	
	/*foreach ($arResult['OFFERS'] as $key => $offer_dest){
		//echo $_REQUEST['NAME_DEST'].'&&'.$offer_dest['NAME'];
		if ((string) $offer_dest['NAME'] === (string)($_REQUEST['NAME_DEST'])) {
	
			$arResult['NEW_OFFERS'][] = $offer_dest;
		}
		
	}*/
	//unset ($arResult['OFFERS']);
	$arResult['OFFERS'] = $arResult['NEW_OFFERS'];
}
...
foreach ($arResult['OFFERS'] as $key => $OfferPartnersDest) {
	foreach ($ArrUsersPartners as $keyUsers => $UsersPartners) {
		if ($UsersPartners['PROPS']['ID_COMPANY']['VALUE'] == $OfferPartnersDest['ID']){
			
			$arResult['OFFERS'][$key]['USERS'][]= $UsersPartners;
		}
	}
}
