...
<form id="rating_form" method="post" action="<?=POST_FORM_ACTION_URI?>">
	<div id="rating_news">
		<div class="rating_news_title"><?=GetMessage("RATING_TITLE")?></div>
		<div value="0" class="rating_news_radio">1</div>
		...
		<div value="4" class="rating_news_radio current">5</div>					
		<div class="rating_news_text"><?=GetMessage("RATING")?><span><?=$DISPLAY_VALUE?></span> (<?=GetMessage("VOTES_COUNT")?><?=$votes_count?>)</div>
		<div class="clear"></div>
	</div>
	<select name="rating" style="display:none;">
		<?foreach($arResult["VOTE_NAMES"] as $i=>$name):?>
			<option value="<?=$i?>"<?if($name=='5')echo' selected';?>><?=$name?></option>
		<?endforeach?>
	</select>
	<?echo bitrix_sessid_post();?>
	<input type="hidden" name="back_page" value="<?=$arResult["BACK_PAGE_URL"]?>" />
	<input type="hidden" name="vote_id" value="<?=$arResult["ID"]?>" />
	<input class="btn_estimate" type="submit" name="vote" value="<?=GetMessage("VOTE")?>" />
	<div class="clear"></div>
</form>
<?/*
