...
<div class="bx_section">
	...
	<div id="sale_order_props" <?=($bHideProps && $_POST["showProps"] != "Y")?"style='display:none;'":''?>>
		...
		PrintPropsForm2($arResult["ORDER_PROP"]["USER_PROPS_N"], $arParams["TEMPLATE_LOCATION"]);
		PrintPropsForm2($arResult["ORDER_PROP"]["USER_PROPS_Y"], $arParams["TEMPLATE_LOCATION"]);
		include($_SERVER["DOCUMENT_ROOT"].SITE_TEMPLATE_PATH.'/components/bitrix/sale.order.ajax/new_order/delivery.php');
		PrintPropsForm3($arResult["ORDER_PROP"]["USER_PROPS_N"], $arParams["TEMPLATE_LOCATION"]);
		PrintPropsForm3($arResult["ORDER_PROP"]["USER_PROPS_Y"], $arParams["TEMPLATE_LOCATION"]);
		PrintPropsForm4($arResult["ORDER_PROP"]["USER_PROPS_N"], $arParams["TEMPLATE_LOCATION"]);
		PrintPropsForm4($arResult["ORDER_PROP"]["USER_PROPS_Y"], $arParams["TEMPLATE_LOCATION"]);
		?>
	</div>
	<div id="sale_order_info">
		<p class="marg_bot20"><span class="red">*</span> - <?=GetMessage("PROPS_REQUIRED")?></p>
		<p><?=GetMessage("PROPS_IF_REQ")?></p>
		<ul>
			<li>- <?=GetMessage('PROP_COMP_NAME')?></li>
			<li>- <?=GetMessage('PROP_CONTACT')?></li>
			<li>- <?=GetMessage('PROP_PHONES')?></li>
			<li>- E-mail;</li>
			<li>- <?=GetMessage('PROP_DELIVERY_BLOCK')?></li>
		</ul>
		<p class="marg_bot20"><?=GetMessage("PROPS_ELSE_REQ")?></p>
		<p><?=GetMessage("PROP_COMMENT")?></p>
	</div>
	<div class="clear"></div>
	<div class="confirm_btns">
		<a href="<?=SITE_DIR?>personal/cart" class="update_data"><?=GetMessage("TO_BASKET")?></a>
		<input type="button" value="<?=GetMessage("TO_CONFIRM")?>" class="submitbutton" id="checkout_btn">
		<div class="clear"></div>
	</div>
</div>
