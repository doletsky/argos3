...
<div class="login_page">
	...
	<h2 style="float:left;"><?echo GetMessage("AUTH_TITLE")?></h2>
	<a style="float:right;" id="registration_form_link" href="<?=SITE_DIR?>registration_form"><?echo GetMessage("REGISTRATION_FORM_LINK")?></a>
	<div class="clear"></div>
	<?//endif?>
	<?if($arResult["AUTH_SERVICES"]):
	...
	<form name="form_auth" method="post" target="_top" action="<?=SITE_DIR?>auth/<?//=$arResult["AUTH_URL"]?>" class="bx_auth_form">
		...
		<?endif*/?>
		<br><input type="submit" name="Login" class="bt_blue big shadow" value="<?=GetMessage("AUTH_AUTHORIZE")?>" />
	</form>
</div>
<script type="text/javascript">
