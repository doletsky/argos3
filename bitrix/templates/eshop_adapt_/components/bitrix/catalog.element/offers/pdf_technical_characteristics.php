...
<?php if($arResult['PDF']):?>					
						<?if(IsIE() || (strpos($_SERVER['HTTP_USER_AGENT'], 'like Gecko') > 0 && !strpos($_SERVER['HTTP_USER_AGENT'], 'Chrome'))){?>
							...
							<a href="http://<?=$_SERVER['SERVER_NAME'];?><?=$arResult['PDF']?>" class="IframeLink" style="display: none;"></a>
						<?}else{?>
							...
							<iframe src="https://docs.google.com/gview?url=http://<?= ($_SERVER['SERVER_NAME'] . $arResult['PDF'])?>&embedded=true" name="iframeins" width="100%" style="width: 100%;min-height:2000px;" class="iframeins"></iframe>
