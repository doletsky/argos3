<?if(CModule::IncludeModule("sale")) {
		...
		if($cur=='RUB'){
			...
			$html=$html.'<h1>Order №'.$ORDER_ID.'</h1>';
		}
		$html=$html.'
		<div style="width:650px;border: solid 1px #e2e2e2;">
			<table style="border: solid 1px #f3f3f3;border-top: none;border-bottom: none;width:650px;">
				<tr class="cart_title">
					...
					<td class="cart_title_td price confirm">'.GetMessage("PRICE_UNIT").'<br>(c НДС)</td>
					<td class="cart_title_td cost confirm">'.GetMessage("PRICE").'<br>(c НДС)</td>
				</tr>
			</table>
		</div>';
		/*$html=$html.'
		...
		if($cur=='RUB'){
			$html=$html.'			
			<div class="confirm_text_info">В стоимость включена доставка до выбранного Вами терминала в Санкт-Петербурге<br><br>Услуги по доставке товара, выбранной транспортной компанией, оплачиваются клиентом самостоятельно</div>';
		}
	...
	file_put_contents($_SERVER["DOCUMENT_ROOT"]."/dompdf_lib/tmp_gen/zakaz_".$ORDER_ID.".pdf", $output); 
