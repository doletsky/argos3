...
<div id="order_confirm">
	...
	<div class="cart_title">
		...
		<div class="cart_title_td cost confirm"><?=GetMessage('PRICE')?> <br>(с НДС)</div>
	</div>
	<?
	$count_items=0;
	$packs = 0;
	...
	foreach ($arResult["GRID"]["ROWS"] as $k => $arData)
	{
		...
		if(empty($deviant_packing) || $deviant_packing == ' ')	$deviant_packing = intval(1);
		$count_items = $count_items + $arData["data"]["QUANTITY"];/**$deviant_packing;*/
		$weight = $weight + $arData["data"]["WEIGHT"];
		$dim = $arData["data"]["DIMENSIONS"]["WIDTH"] * $arData["data"]["DIMENSIONS"]["HEIGHT"] * $arData["data"]["DIMENSIONS"]["LENGTH"];
		$volume = $volume + $dim;		
		...
		<div class="cart_items">
			<div class="cart_item">			
				...
				<div class="cart_item_td quantity confirm"><?=$arData["data"]["QUANTITY"]/**$deviant_packing*/?> шт.</div>
				<?
				...
				$days = intval($days);
				if($days == 1){
					$shiping_days = 'на следующий день после оплаты';
				}else{
					$shiping_days = 'в течение '.$days.'дней после оплаты';
				}
				?> <?//=GetMessage("SOA_TEMPL_DAYS", Array("#DAYS#"=>$days));?>
				<div class="cart_item_td shipping confirm"><?=$shiping_days?></div>
	...
	<div id="cost_block_confirm">
		...
		<?php if($arResult['PRICE_WITHOUT_DISCOUNT'] != $arResult["ORDER_TOTAL_PRICE_FORMATED"]):?>
		<div class="cost_itogo">Сумма: <span class="old_price"><?php echo $arResult['PRICE_WITHOUT_DISCOUNT']?></span></div>
		<span class="discount">Ваша скидка <?php echo $arResult['DISCOUNT_PRICE_ALL_FORMATED']?></span>
		<?php endif;?>
	...
	<div id="data_order">
		<div class="data_order_wrap">
			...
			<ul>
				...
				<?}?>
				<?if(SITE_DIR=='/') {?>
					<li>КПП: <span class="confirm_order_prop" id="<?echo 'CONFIRM_ORDER_PROP_49';?>"></span></li>
				<?}?>
		...
		<div class="data_order_wrap">
			...
			<?}else{
		//	pre_debug($arResult["DELIVERY"]);
			
				foreach($arResult["DELIVERY"] as $k=>$v){
					if($v['CHECKED']){
						$delivery_name = $arResult["DELIVERY"][$k]["NAME"];
					}
				}
			?>
				<div class="data_order_title"><?=GetMessage("CONF_DELIVERY_DATA")?></div>
				<ul>			
					<li><span class="confirm_order_prop"><?=$delivery_name?></span></li>		
				</ul>	
			<?}?>
		</div>
		<?if(SITE_DIR=='/') {?>
