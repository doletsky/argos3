...
<?if($arResult["ALLOW_ANONYMOUS"]=="Y" && $_REQUEST["authorize"]<>"YES" && $_REQUEST["register"]<>"YES"):?>
	<div class="title"><?echo GetMessage("subscr_title_auth2")?></div>
...
<?elseif($arResult["ALLOW_ANONYMOUS"]=="N" || $_REQUEST["authorize"]=="YES" || $_REQUEST["register"]=="YES"):?>
	...
	<div class="title"><?echo GetMessage("adm_auth_exist")?></div>
	<?if($arResult["ALLOW_ANONYMOUS"]=="Y"):?>
		<p><?echo GetMessage("subscr_auth_note")?></p>
	<?else:?>
		<p><?echo GetMessage("adm_must_auth")?></p>
	<?endif;?>
	<p>
		<div class="title_req"><?echo GetMessage("adm_auth_login")?><span class="starrequired">*</span></div>
		<input class="input_style" type="text" name="LOGIN" value="<?echo $arResult["REQUEST"]["LOGIN"]?>" size="20" />
	</p>
	<p>
		<div class="title_req"><?echo GetMessage("adm_auth_pass")?><span class="starrequired">*</span></div>
		<input class="input_style" type="password" name="PASSWORD" size="20" value="<?echo $arResult["REQUEST"]["PASSWORD"]?>" />
	</p>
	<input class="submit_style" type="submit" name="Save" value="<?echo GetMessage("adm_auth_butt")?>" />
	...
	<?if($arResult["ALLOW_REGISTER"]=="Y"):
		...
		<form action="<?=$arResult["FORM_ACTION"]?>" method="post">
		<?echo bitrix_sessid_post();?>
		<div class="title"><?echo GetMessage("adm_reg_new")?></div>
		<?if($arResult["ALLOW_ANONYMOUS"]=="Y"):?>
			<p><?echo GetMessage("subscr_auth_note")?></p>
		<?else:?>
			<p><?echo GetMessage("adm_must_auth")?></p>
		...
		<p>
			<div class="title_req"><?echo GetMessage("adm_reg_login")?><span class="starrequired">*</span></div>
			<input class="input_style" type="text" name="NEW_LOGIN" value="<?echo $arResult["REQUEST"]["NEW_LOGIN"]?>" size="20" />
		</p>
		<p>
			<div class="title_req"><?echo GetMessage("adm_reg_pass")?><span class="starrequired">*</span></div>
			<input class="input_style" type="password" name="NEW_PASSWORD" size="20" value="<?echo $arResult["REQUEST"]["NEW_PASSWORD"]?>" />
		</p>
		<p>
			<div class="title_req"><?echo GetMessage("adm_reg_pass_conf")?><span class="starrequired">*</span></div>
			<input class="input_style" type="password" name="CONFIRM_PASSWORD" size="20" value="<?echo $arResult["REQUEST"]["CONFIRM_PASSWORD"]?>" />
		</p>
		<p>
		...
		if (COption::GetOptionString("main", "captcha_registration", "N") == "Y"):
			...
			?>
			<p>
				<div class="title_req"><?=GetMessage("subscr_CAPTCHA_REGF_TITLE")?></div>
			...
			<p>
				<div class="title_req"><?=GetMessage("subscr_CAPTCHA_REGF_PROMT")?><span class="starrequired">*</span></div>
				<input class="input_style" type="text" name="captcha_word" size="30" maxlength="50" value="" />
			</p>
		<?endif;?>
		<input class="submit_style" type="submit" name="Save" value="<?echo GetMessage("adm_reg_butt")?>" />
