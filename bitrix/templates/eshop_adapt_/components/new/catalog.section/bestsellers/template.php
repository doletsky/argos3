...
//PROPERTY_BESTSELLER_VALUE
        ...
        $marginClass = ($i % 3 == 0) ? 'no-margin' : '';
        ?>
	<div class="<? echo($arItem['SECOND_PICT'] && $arParams ? 'bx_catalog_item double ' : 'bx_catalog_item '); echo $marginClass;?>" id="<? echo $strMainID; ?>">
	<?
	...
	<a id="<?=$arItemIDs['PICT']; ?>" target="_blank" href="<?=$arItem['DETAIL_PAGE_URL']; ?>" class="bx_catalog_item_images" style="background-image: url('<?=$arItem['PREVIEW_PICTURE']['SRC']?>');<?=$StyleImg;?>" title="<?=$strTitle; ?>">
	...
	<div class="bx_catalog_item_title">
		<a target="_blank" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $arItem['NAME']; ?>"><?=$arItem['NAME']; ?></a>
	</div>
...
var <? echo $strObName; ?> = new JCCatalogSectionBest(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	<?
	$boolShowOfferProps = 0;//!!$arItem['OFFERS_PROPS_DISPLAY'];
	$boolShowProductProps = 0;//(isset($arItem['DISPLAY_PROPERTIES']) && !empty($arItem['DISPLAY_PROPERTIES']));
