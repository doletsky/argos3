...
.bx_content_section ul, ol ul {
    list-style: initial;
    padding-left: 20px;
    margin-bottom: 5px;
}

.bx_content_section li, ol ul li {
   margin-top: 5px; 
   margin-bottom: 5px; 
}
...
if(!in_array($arResult['CODE'],$arSECTION_CODE ) ){
	$arResult['ITEMS'] = $arResult['ITEMS_OFFER'];
}
*/
$nav = CIBlockSection::GetNavChain(false, $arResult['IBLOCK_SECTION_ID']);
    while($arItem = $nav->Fetch()){
        $ITEMS[] = $arItem[ID];
        $ITEMS_CODE = $arItem[CODE];
    }
 //print_r($ITEMS_CODE);    
foreach ($arResult['ITEMS'] as $key => $arItem)
{
    if ($ITEMS>0) {
        $arItem['DETAIL_PAGE_URL']='/catalog/'.$ITEMS_CODE.'/'.$arItem[CODE].'/';
    ...
    if ($arItem['PROPERTIES']['CATALOG_NAME']['VALUE']>''){
    $arItem['NAME']=$arItem['PROPERTIES']['CATALOG_NAME']['VALUE'];
    $productTitle=$arItem['PROPERTIES']['CATALOG_NAME']['VALUE'];
    $imgTitle=$arItem['PROPERTIES']['CATALOG_NAME']['VALUE'];    
    }

    $marginClass = ($key % 3 == 0) ? 'no-margin' : '';
    if ($arItem['PROPERTIES']['CATALOG_NAME']['VALUE']>''){
    $arItem['NAME']=$arItem['PROPERTIES']['CATALOG_NAME']['VALUE'];
    $productTitle=$arItem['PROPERTIES']['CATALOG_NAME']['VALUE'];
    $imgTitle=$arItem['PROPERTIES']['CATALOG_NAME']['VALUE'];    
	...
	<?
	/*ОБХОД CSS background-size для IE*/
	if(IsIE() || (strpos($_SERVER['HTTP_USER_AGENT'], 'like Gecko') > 0 && !strpos($_SERVER['HTTP_USER_AGENT'], 'Chrome'))){
		
			$StyleImg = "filter: progid:DXImageTransform.Microsoft.AlphaImageLoader( src='".$arItem['PREVIEW_PICTURE']['SRC']."', sizingMethod='scale');
			    -ms-filter: progid:DXImageTransform.Microsoft.AlphaImageLoader( src='".$arItem['PREVIEW_PICTURE']['SRC']."', sizingMethod='scale');";
			
	}
	/*---END---*/
	?> 
			    
	<div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double ' : 'bx_catalog_item '); echo $marginClass;?> "><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
		<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" target="_blank" class="bx_catalog_item_images" style="background-image: url('<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>');<?=$StyleImg?>" title="<? echo $imgTitle; ?>">
		
		
		<?
	if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT'] && ($arItem['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'] > 0))
	{
	?>
	...
	{
	?>
			<div id="<? echo $arItemIDs['STICKER_ID']; ?>" class="bx_stick average left top" title="<? echo $arItem['LABEL_VALUE']; ?>"></div>
	<?
	}
	?>
		</a><?
	...
	{
		?><a id="<? echo $arItemIDs['SECOND_PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" target="_blank" class="bx_catalog_item_images_double" style="background-image: url('<? echo (
		...
		{
		?>
			<div id="<? echo $arItemIDs['SECOND_STICKER_ID']; ?>" class="bx_stick average left top" title="<? echo $arItem['LABEL_VALUE']; ?>"></div>
		<?
		}
		?>
		</a><?
	...
	?>
	<div class="bx_catalog_item_title sim-height">
		<a href="<?=$arItem['DETAIL_PAGE_URL']; ?>" title="<?=$productTitle; ?>"><?=$arItem['NAME']; ?></a>
	</div>
	...
	{ 
		...
		{
			if($arItem['MIN_PRICE']['VALUE'] <> 0) echo $arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE'];
		}
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		if ($arItem['CAN_BUY'] && $arItem['MIN_PRICE']['VALUE'] <> 0)
		{ 
		...
		{ 
			//echo '<pre>',print_r($arItem['MIN_PRICE']),'</pre>';
			//if($arItem['MIN_PRICE']['VALUE'] <> 0)
...
<?
		...
		//echo ('Y' == Y ? $arItemIDs['SECOND_PICT'] : $arItemIDs['PICT']);
		//echo "<pre>",print_r($arItem['SECOND_PICT']),"<pre>";
		$arJSParams = array(
...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	else
	{
		if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
		{
			...
			?>
		<div class="catalog_item_controls no_touch">
			<?
			if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
			{
			?>
