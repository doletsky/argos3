...
//echo "<pre>",var_dump($arParams["SECTION_CODE"]),"</pre>";
$arSECTION_CODE2 = array(
		0 => 'draivery',
		1 => '35_vt',
		2 => '40_vt',
		3 => '50_vt',
...
if(!in_array($arParams["SECTION_CODE"],$arResult['arSECTION_CODE'] ) && in_array($arParams["SECTION_CODE"],$arSECTION_CODE2) ){
	$key = 0;
	foreach ($arResult['ITEMS'] as $key=>$items){
		//echo "<pre>",print_r($items),"</pre>";
		$res = CIBlockElement::GetByID($items['PROPERTIES']['MODEL']['VALUE']);
...
foreach ($arResult['ITEMS'] as $key=>$items){
	foreach ($items['PROPERTIES'] as $key=>$Prop){
		if(in_array($Prop['ID'],$Props)){
		$Props[] = $Prop['ID']."<br>";
		}
	}
}
