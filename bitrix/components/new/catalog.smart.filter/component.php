...
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		...
		else
		{
			...
			$arSECTION_ID = array(26,319,185,298);

			if(!in_array($this->SECTION_ID,$arSECTION_ID)){
				//unset($arElementFilter['SUBSECTION']);
			}
//			echo $this->IBLOCK_ID;
			...
			if (!empty($this->arResult["PROPERTY_ID_LIST"]))
			{
				...
				while($arElement = $rsElements->Fetch())
				{
					...
					{
						if (isset($arElement[$key]) && $arElement[$key] !== false ){
							$arNewElement[$key] = $arElement[$key];
						}
					}
					$arElements[$arElement["IBLOCK_ELEMENT_ID"]] = $arNewElement;
   					unset($arElement);
				}
			}
			else
...
if(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y"){
	...
	$arSECTION_CODE = array('moduli','seriya_seoul','seriya_epistar','promyshlenno_ulichnye_moduli');
	
	if(!in_array($arParams['arSectModul'],$arSECTION_CODE)){
		$rsElements = CIBlockElement::GetList(array(), $arFilter, false, false, array());
		while($arItem = $rsElements->GetNext()){
			$SectItems[] = (int)$arItem['ID'];

		}
		//echo "<pre>",print_r($arFilter),"</pre>";
		if(isset($arFilter['=ID'])){
			$arFilter2 = $arFilter['=ID']->arFilter;
		}else{
			$arFilter2 = $arFilter[0]['=ID']->arFilter;
		}
		$arFilter2["PROPERTY_MODEL"] = $SectItems;
		$arFilter = $arFilter2;
	}
