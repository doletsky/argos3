...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	$arSECTION_CODE = array(
			...
			2 => 'seriya_epistar',
			3 => 'seriya_edisson',
			4 => 'promyshlenno_ulichnye_moduli',
	);
	
	if(!in_array($arParams["SECTION_CODE"], $arSECTION_CODE ) ){
		/*$arFilter = array(
				"IBLOCK_ID" => $arParams["IBLOCK_ID"],
				"IBLOCK_LID" => SITE_ID,
				"IBLOCK_ACTIVE" => "Y",
		...
		$arFilter = array(
			...
			"IBLOCK_ID" => "4",
			"ACTIVE" => "Y",
			"PROPERTY_MODEL" => $SectItems,
			"CATALOG_SHOP_QUANTITY_1" => true,
		);
		if(isset($arrFilter['OFFERS']) && !empty($arrFilter['OFFERS']))	{
			$arrFilter = $arrFilter['OFFERS'];
		}
		//echo "<pre>",print_r($arrFilter['OFFERS']),"</pre>";
	}
	//echo "<pre>",print_r($SectItems),"</pre>";
	$arDefaultMeasure = array();
	if ($bIBlockCatalog)
		$arDefaultMeasure = CCatalogMeasure::getDefaultMeasure(true, true);
	$arCurrencyList = array();
	$arSections = array();
	$arFilter['>CATALOG_PRICE_1'] = 0;
	//$arFilter['CATALOG_SHOP_QUANTITY_1'] = 1;
	//echo "<pre>",print_r(array_merge($arrFilter, $arFilter)),"</pre>";
	//EXECUTE
	...
	while($arItem = $rsElements->GetNext())
	{
		...
		//echo "<pre>",print_r($arItem['ID']),"</pre>";
	...
	$arResult['arSECTION_CODE'] = $arSECTION_CODE;
