...
define("EDOST_ID", 4802); //Идентификатор магазина (вписать вместо: 0)
define("EDOST_PSWD", "Ip3VwiMdHrvMdrlncrogVj73r8OVIj6T"); //Пароль для доступа к серверу расчетов (вписать вместо: password)
//---------- ВНИМАНИЕ!!! ----------

define("EDOST_MAX_CODE", 2297); //максимальный код eDost
...
Версия: 1.10 от 30.04.2014
	...
	function edost_calc($to_city, $weight, $strah, $length, $width, $height, $zip, $from_city) - функция расчета стоимости доставки
		1. $from_city - код города откуда неоходимо отправить посылку (используется только на специальном тарифном плане)
		2. $to_city - город или страна, куда неоходимо отправить посылку (название на русском языке в кодировке windows-1251)
		3. $weight - вес в кг
		4. $strah - сумма для страховки в рублях
		5. $length - длина посылки (можно не указывать)
		6. $width  - ширина посылки (можно не указывать)
		7. $height - высота посылки (можно не указывать)
		8. $zip - индекс (можно не указывать)
...
var $site_win = false; //если 1, тогда перекодировать в WIN (utf8_win) - это результаты расчетов
var $index, $curElem;
var $status = 0;
var $parser;
var $rz;
...
var $test=0; //включить тестовый вывод =1
	...
	function edost_calc($to_city, $weight, $strah, $length, $width, $height, $zip='', $from_city='') {
		...
		$from_city = preg_replace("/[^0-9]/i","", substr($from_city,0,5));
		...
		if ( ($to_city==$to_city_withi) and ($to_city_id<>'') ) {
			...
			if ( $to_city_id<1 || $to_city_id>EDOST_MAX_CODE )
		...
		$xml = $this -> ManualPOST ($this->adr, "to_city=$to_city&strah=$strah&id={$this->edost_id}&p={$this->edost_pswd}&weight=$weight&ln=$length&wd=$width&hg=$height&zip=$zip&from_city=$from_city");

		if ($this->test==1) echo '<br>=12======<br>('.$xml.')'.$this->adr;
	...
	function edost_calc_post() {
		...
		if (	(isset($_POST['edost_to_city'])) and
			...
			$from_city = ''; //здесь можно задать код города откуда неоходимо отправить посылку (используется только на специальном тарифном плане)
			...
			$to_city = trim(preg_replace("/[^a-z0-9а-яА-Я-(),.' ]/iu","", substr($_POST['edost_to_city'],0,100))); //город доставки
			...
			//echo '<br>strah=',$strah.' , to_city=',$this->win_utf8($to_city).' , weight=',$weight;
			$r = $this -> edost_calc($to_city, $weight, $strah, $length, $width, $height, $zip, $from_city);
